...
for ii = 1:size(xs,1)
    ...
    incdb(:,ii) = aux.incdb;  % Null coverage from 2005 
    incdc(:,ii) = aux.incdc;  % Constant coverage from 2005 
    ...
    mord(:,ii) = aux.mortd;  % Null coverage at 2005
    more(:,ii) = aux.morte;  % Constant coverage at 2005
    ...
    mor1(:,ii)= ((aux.mort(9:end))./1e5).*pop2005_2022'; 
    morb1(:,ii) = ((aux.mortb)./1e5).*pop2000_2022';
    morc1(:,ii) = ((aux.mortc)./1e5).*pop2000_2022';
    mord1(:,ii) = ((aux.mortd)./1e5).*pop2005_2022';
    more1(:,ii) = ((aux.morte)./1e5).*pop2005_2022';
    ...
    % Lives saved by null coverge (LS3) and by cont coverage (LS4) from 2005
    LS3(:,ii) = (mord1(:,ii) - mor1(:,ii)); % 2005-2022
    LS4(:,ii) = (more1(:,ii) - mor1(:,ii)); % 2005-2022
...
incdb_pct = prctile(incdb,[2.5,50,97.5],2);  %2005-2019 % Null coverage
incdc_pct = prctile(incdc,[2.5,50,97.5],2);  %2005-2019 % Constant Coverage
...
morb_pct = prctile(morb,[2.5,50,97.5],2);  %2005-2019 % Null coverage
morc_pct = prctile(morc,[2.5,50,97.5],2);  %2005-2019 % Constant Coverage
...
LS3_pct = prctile(LS3,[2.5,50,97.5],2); % 2005-2022
LS4_pct = prctile(LS4,[2.5,50,97.5],2); % 2005-2022
...
plt = mord_pct(1:end,:)'; % Null coverage from 2005
lg(4,:) = plot(2005:2022, plt(2,:),'y','LineWidth',1.5);  %Mortality 2005:2022
jbfill(2005:2022, plt(3,:), plt(1,:), 'y', 'None', 1, 0.3); hold on

plt = more_pct(1:end,:)'; % Constant coverage from 2005
lg(5,:) = plot(2005:2022, plt(2,:),'m','LineWidth',1.5);  %Mortality 2005:2022
jbfill(2005:2022, plt(3,:), plt(1,:), 'm', 'None', 1, 0.3); hold on
...
yl = ylim; yl(1) = 0; ylim(yl);
xlim([2000 2035]);
legend(lg(1:7,:),'Current trend','Null services from 2000','Constant coverage from 2000','Null services from 2005','Constant coverage from 2005','Null services from 2022','Constant coverage from 2022')
...
plt = incdd_pct(1:end,:)'; 
lg(4,:) = plot(2005:2022, plt(2,:),'y','LineWidth',1.5);  %incidence2005:2022
jbfill(2005:2022, plt(3,:), plt(1,:), 'y', 'None', 1, 0.3); hold on

plt = incde_pct(1:end,:)'; 
lg(5,:) = plot(2005:2022, plt(2,:),'m');  %incidence2005:2022
jbfill(2005:2022, plt(3,:), plt(1,:), 'm', 'None', 1, 0.3); hold on
...
plt = LS3_pct(1:end,:)';  %end-14
lg(3,:) = plot(2005:2022, plt(2,:),'g','LineWidth',1.5);  % LS from 2005:2022 (null covegare)
jbfill(2005:2022, plt(3,:), plt(1,:), 'g', 'None', 1, 0.3); hold on

plt = LS4_pct(1:end,:)';
lg(4,:) = plot(2005:2022, plt(2,:),'b','LineWidth',1.5);  % LS from 2005:2022 (constant covegare)
jbfill(2005:2022, plt(3,:), plt(1,:), 'b', 'None', 1, 0.3); 


set(gca,'fontsize',12);
ylabel('Annual lives saved','fontsize',14);
legend(lg(1:4,:), 'Null services(from 2000)','Constant coverage (from 2000)','Null service(from 2005)','Constant coverage (from 2005)','Location','NorthEast');
yl = ylim; yl(1) = 0; ylim(yl);

function [out, aux] = get_livessaved(x, prm, ref, sel, agg, gps,gps1,gps2,calfn)     %India
%x = [10.8199494614031,0.922013812587250,0.380366315609487,27.9989805416612,0.261552592306905,2.13266298020697,23.2375220939466,0.313648234824761,0.800141532707012,0.844860058081694,0.105302604292388,0.757587212828534,0.760614080319824,0.0139165734172480,0.0104657332274439,1.43437476055268,0.00824735341997097,0.161891366706001,79.2727675547669,1.08974268747507,1.71556794160624,0.937688262891094];
...
%     keyboard;
  ...
  % Fixing the intervention parameters in 2000
    ...
    MC2000 = make_model2(p6, r6, i, s, gps,prm); % constant coverage
     ...
     % Condition in 2002
    p2 = p; r2 = r;
    p2.pu = 0 + (p.pu_2022 - 0)*((2002-1997)/25);  % Assuming linear scaleup from 1997 to 2022
    r2.cs  = r.cs_1997 + (r.cs_2022 - r.cs_1997)*((2002-1997)/25);
    r2.cs2 = r.cs2_1997 + (r.cs2_2022 - r.cs2_1997)*((2002-1997)/25); % Assumed to be same from 1997 to 2022
    p2.MDR_rec = [0 0]; %r2.ART_init = 0;        
    M2b = make_model2(p2, r2, i, s, gps,prm);

   % In absense of program (removal of program in 2003 and back to 1997 condition)
    p7 = p; r7 = r;
    p7.pu = 0;  
    ...
    MN2003 = make_model2(p7, r7, i, s, gps,prm); % Null coverage

    % Fixing the intervention parameters in 2003
    p8 = p; r8 = r;
    p8.pu = 0 + (p.pu_2022 - 0)*((2003-1997)/25);  % Assuming linear scaleup from 1997 to 2022
    r8.cs  = r.cs_1997 + (r.cs_2022 - r.cs_1997)*((2003-1997)/25);
    r8.cs2 = r.cs2_1997 + (r.cs2_2022 - r.cs2_1997)*((2003-1997)/25); % Assumed to be same from 1997 to 2022
    p8.MDR_rec = [0 0]; %r8.ART_init = 0;         
    MC2003 = make_model2(p8, r8, i, s, gps,prm); % constant coverage
  ...
  % Fixing the intervention parameters in 2000
   ...
   % With constant coverage as in 2000
    ...
    [tc, solnc] = ode15s(geq, [2000:1:2023], init2000, odeset('NonNegative',[1:i.nstates]));
     ...
     % Without program from 2003
     prm.growth = prm.gth(3);betared = (r.betared)^max(2003-2000, 0); %0.5961; 
     %geq = @(t,in)goveqs_basis2(t, in, r7, MN2005, i, prm, sel, agg);  
     geq = @(t,in) goveqs_scaleup0(t, in, r7, M2b, MN2003, [2003 2023], i, prm, sel, agg,betared); % Arbitary scale up
    [td, solnd] = ode15s(geq, [2003:1:2023], init2003, odeset('NonNegative',[1:i.nstates]));
     
    % Baseline with constant coverage as in 2003
    prm.growth = prm.gth(3);betared = (r.betared)^max(2003-2000, 0); %0.5961;
    %geq = @(t,in)goveqs_basis2(t, in, r8, MC2005, i, prm, sel, agg);  
    geq = @(t,in) goveqs_scaleup0(t, in, r8, M2b, MC2003, [2003 2023], i, prm, sel, agg,betared); % Arbitary scale up
    [te, solne] = ode15s(geq, [2003:1:2023], init2003, odeset('NonNegative',[1:i.nstates]));
    ...
    soln_d = interp1(td, solnd, 2003:2023); % With null coverage from 2005
    dsold  = diff(soln_d,[],1);

    soln_e = interp1(te, solne, 2003:2023); % With constant coverage from 2005
    dsole  = diff(soln_e,[],1);
    ...
    popf = sum(solng(1:end-1,1:i.nstates),2);  %2022:2035 (1:end -> 2022:2036) % forward population projection
      ...
      incd_d = (dsold(:,i.aux.inc(1))./pop(7:end))*1e5; % 2003-2022
      incd_e = (dsole(:,i.aux.inc(1))./pop(7:end))*1e5; % 2003-2022
      ...
      mort_d = (dsold(:,i.aux.mort(1))./pop(7:end))*1e5;% 2003-2022
      mort_e = (dsole(:,i.aux.mort(1))./pop(7:end))*1e5;% 2003-2022

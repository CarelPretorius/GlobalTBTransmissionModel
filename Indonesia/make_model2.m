function M = make_model(p, r, i, s, gps,prm)  % India
...
for istr = 1:length(gps.strains)
    ...
    for ih = 1:length(gps.hiv)
        ...
        source = I; destin = Dxs.pu; rate = r.cs(ia)*p.pu + r.access;
        ...
        source = E; destin = Dxs.pu; rate = r.cs2(ia)*p.pu;
        m2(destin, source) = m2(destin, source) + rate;
        ...
        source = Isc; destin = Dxs.pu; rate = r.cs3(ia)*p.pu ;
        m2(destin, source) = m2(destin, source) + rate;
       ...
       % --- LTBI cure excluding hiv component r.LTBI_cure(ih) = [rate 0 0]
%     for ih = 1
%         sources = [Lf, Ls];
%         destin  = i.U.(age).(hiv).v0;
%         rates   = r.LTBI_cure;
